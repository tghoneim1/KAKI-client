...
const PRODUCTS = [
  { id:"shamoort",     name:"فرخة شامورط (أقل من كيلو)", emoji:"🐣", price:150, unit:"ك" },
  { id:"whole",        name:"فرخة كاملة",                emoji:"🐔", price:190, unit:"ك" },
  { id:"breast_full",  name:"صدور كاملة",                emoji:"🥩", price:250, unit:"ك" },
  { id:"breast_deb",   name:"صدور مخلية",                emoji:"🥩", price:390, unit:"ك" },
  { id:"fillet",       name:"صدور فيليه (بانيه)",        emoji:"🥓", price:390, unit:"ك" },
  { id:"wings",        name:"وراك كاملة",                emoji:"🍗", price:100, unit:"ك" },
  { id:"shish",        name:"شيش طاوق",                  emoji:"🍢", price:390, unit:"ك" },
  { id:"shawarma",     name:"شاورمة فراخ",               emoji:"🌯", price:390, unit:"ك" },
];
...
export default function ClientOrderForm(){
  ...
  const sendOrder=async()=>{
    ...
    const fullAddress=delivery==="pickup"?"استلام شخصي":`${gov} — ${area} — ${street} — ${building}${aptNum?" ش"+aptNum:""}${floor?" د"+floor:""}${extra?" — "+extra:""}`;
    const order={
      id:"ORD-"+Date.now(),
      client:fullName,
      phone,
      address:fullAddress,
      gov,area,street,building,aptNum,floor,
      delivery,
      items,
      total,
      notes:"",
    ...
    try{
      // Save to Firebase
      const res=await fetch(`${FIREBASE_URL}/orders/${order.id}.json`,{
        method:"PUT",
  ...
  return(
    <div style={{fontFamily:"'Cairo',sans-serif",background:BG,minHeight:"100vh",color:CREAM,direction:"rtl",maxWidth:400,margin:"0 auto"}}>
      ...
      {/* Header */}
      <div style={{background:"linear-gradient(135deg,#3D1A00,#5C2A00)",padding:"16px 18px",textAlign:"center",borderBottom:"1px solid #6B3500",position:"sticky",top:0,zIndex:50}}>
      ...
      <div style={{display:"flex",padding:"10px 16px",gap:4,background:CARD,borderBottom:`1px solid ${BDR}`}}>
        {[["١","بياناتك"],["٢","العنوان"],["٣","طلبك"],["٤","تأكيد"]].map(([n,l],i)=>(
          <div key={i} style={{flex:1,textAlign:"center"}}>
            <div style={{width:24,height:24,borderRadius:"50%",background:step>i+1?GOLD:step===i+1?GOLD:LIGHT,color:step>=i+1?"#fff":MUT,fontWeight:800,fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 3px"}}>
              {step>i+1?"✓":n}
            </div>
      ...
      <div style={{padding:16,paddingBottom:100}}>
        ...
        {step===2&&(
          <div>
            ...
            <button onClick={()=>setMapOpen(o=>!o)}
              style={{width:"100%",background:locSet?"#10b98122":"#111",border:`1px solid ${locSet?"#10b981":BDR}`,borderRadius:12,padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",marginBottom:10,fontFamily:"'Cairo',sans-serif",color:CREAM}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:22}}>🗺️</span>
                <div style={{textAlign:"right"}}>
                  <div style={{fontWeight:700,fontSize:13,color:locSet?"#10b981":CREAM}}>{locSet?"✅ تم تحديد الموقع على الخريطة":"تحديد الموقع على الخريطة"}</div>
            ...
            {mapOpen&&(
              <div style={{marginBottom:10,borderRadius:12,overflow:"hidden",border:`1px solid ${BDR}`}}>
                ...
                <div style={{padding:"8px 10px",background:"#3D1A00",display:"flex",gap:8}}>
                ...
                <div style={{fontSize:10,color:MUT,textAlign:"center",padding:5,background:"#3D1A00"}}>اضغط على الخريطة أو اسحب الـ Pin</div>
              </div>
            )}
            ...
            <div style={{display:"flex",gap:6,marginBottom:8}}>
              {Object.keys(AREAS).map(g=>(
                <button key={g} onClick={()=>{setGov(g===gov?"":g);setArea("");setErrors(r=>({...r,gov:false}));}}
                  style={{flex:1,background:gov===g?(GOV_COLORS[g]||GOLD):"#111",color:gov===g?"#000":MUT,border:`1px solid ${errors.gov?"#ef4444":gov===g?(GOV_COLORS[g]||GOLD):BDR}`,borderRadius:10,padding:"10px 4px",fontWeight:700,fontSize:10,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
                  {g} {gov===g?"▲":"▼"}
                </button>
              ))}
            </div>
            ...
            {area&&(
              <>
                <div style={{display:"flex",gap:8,marginBottom:10}}>
                  {PROP_TYPES.map(t=>(
                    <button key={t.id} onClick={()=>setPropType(t.id)}
                      style={{flex:1,background:propType===t.id?"#fff":"#111",color:propType===t.id?"#000":MUT,border:`1px solid ${propType===t.id?"#fff":BDR}`,borderRadius:24,padding:"10px 6px",fontWeight:propType===t.id?800:400,fontSize:12,fontFamily:"'Cairo',sans-serif",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
                      {t.icon} {t.label}
                    </button>
                  ))}
                </div>
                ...
                <input style={{...inp("street"),marginBottom:10}} placeholder="مثال: شارع الجمهورية" value={street} onChange={e=>{setStreet(e.target.value);setErrors(r=>({...r,street:false}));}}/>
        ...
        {step===3&&(
          <div>
                ...
                return(
                  <div key={p.id} style={{background:CARD,borderRadius:14,padding:"12px 14px",border:`1px solid ${qty>0?GOLD:BDR}`,display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:12,color:GOLD,fontWeight:700,marginTop:2}}>ج.م {p.price} / {p.unit}</div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                      ...
                      <button onClick={()=>{add(p.id);setErrors(r=>({...r,items:false}));}} style={{width:32,height:32,borderRadius:9,border:"none",background:qty>0?GOLD:"#2a2a2a",color:qty>0?"#000":CREAM,fontSize:18,cursor:"pointer",fontWeight:700}}>+</button>
                    </div>
                  </div>
                );
              })}
            ...
            {/* Total */}
            {total>0&&(
              <div style={{background:LIGHT,borderRadius:12,padding:"12px 16px",marginBottom:14,border:"1px solid #3d2200",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {step===4&&(
          <div>
            ...
            <div style={{background:CARD,borderRadius:14,padding:"14px 16px",marginBottom:14,border:`1px solid ${BDR}`}}>
              ...
              {PRODUCTS.filter(p=>items[p.id]>0).map(p=>(
                <div key={p.id} style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"5px 0",borderBottom:`1px solid ${BDR}`}}>
                  <span>{p.emoji} {p.name} × {items[p.id]}{p.unit}</span>
            ...
            <button onClick={sendOrder}
              style={{width:"100%",background:"linear-gradient(135deg,#25D366,#128C7E)",color:"#fff",border:"none",borderRadius:16,padding:"16px",fontWeight:900,fontSize:16,fontFamily:"'Cairo',sans-serif",cursor:"pointer",boxShadow:"0 8px 32px rgba(37,211,102,.4)",marginBottom:8}}>
              📲 إرسال الطلب على واتساب
            </button>
            <button onClick={()=>setStep(3)}
              style={{width:"100%",background:"none",color:MUT,border:"none",padding:"10px",fontWeight:600,fontSize:13,fontFamily:"'Cairo',sans-serif",cursor:"pointer"}}>
              ← رجوع
            </button>
          </div>
        )}
